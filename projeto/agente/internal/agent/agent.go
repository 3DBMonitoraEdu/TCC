...
func (a *Agent) collect() {
	...
	logger.Logger("info", fmt.Sprintf("coletado — CPU: %.1f%% RAM: %.1f%% Disco: %.1f%% Processos: %d", metrics.CPUPercent, metrics.MemPercent, metrics.DiskPercent, len(metrics.Processes)), "agent:collect", nil)
	...
	if err != nil {
		...
		logger.Logger("error", "erro ao sincronizar DNS", "agent:collect", err)
	} else if err := dns.UpdatePolicy(policy.Mode, policy.Domains); err != nil {
		logger.Logger("error", "política DNS recebida é inválida", "agent:collect", err)
	} else {
		logger.Logger("info", fmt.Sprintf("política DNS aplicada: modo %s, domínios %d", policy.Mode, len(policy.Domains)), "agent:collect", nil)
	}
